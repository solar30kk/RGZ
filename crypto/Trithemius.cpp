...
extern "C" {
    ...
    EXPORT void encryptFile(const char* inputPath, const char* outputPath, int key) { 
        ...
        while (inFile.get(ch)) {
            ...
            unsigned char encrypted = static_cast<unsigned char>((byte + key + position) % 256);
    ...
    EXPORT void decryptFile(const char* inputPath, const char* outputPath, int key) {
        ...
        while (inFile.get(ch)) {
            unsigned char byte = static_cast<unsigned char>(ch);
            unsigned char decrypted = static_cast<unsigned char>((byte + 256 - ((key + position) % 256)) % 256);
